...
@tool
def extract_page_content(urls: "str | list[str]", output_types: "str | list[str]" = "markdown,text", extract_depth: str = "basic"):
    """
    ...
    extract_depth: "basic" (default) or "advanced". Use "advanced" when the page is likely
          ...
          extraction is returning incomplete or missing data. Avoid advanced unnecessarily
          as it costs more credits.
    ...
    valid = {"html", "markdown", "text", "links", "media"}
    ...
    async def _extract():
        async with AsyncWebCrawler() as crawler:
            ...
            # arun_many fetches all URLs concurrently — no manual per-URL loop
            crawl_results = await crawler.arun_many(urls=url_list, config=config)
